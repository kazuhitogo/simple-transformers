...
def train():
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            ...
            if train_acc >= ADVANCE_THRESHOLD and stage_idx < len(STAGES) - 1:
                torch.save(model.state_dict(), f"transformer_stage{stage_idx}.pt")
                print(f"  checkpoint saved: transformer_stage{stage_idx}.pt")
                stage_idx += 1
                max_num = STAGES[stage_idx]
                v_min, v_max = val_range(max_num)
                print(f"{'─'*72}")
    ...
    print(f"Model saved to transformer.pt  (final stage: {stage_idx}, max_num: {max_num:,})")
